...
int main()
{
  ...
  Vertex_handle vb = cdt.insert(Point(1,0));
  ...
  Vertex_handle vv = cdt.insert(Point(0, 0));
  if( vv == va)
      std::cout << "yes, they are same" << std::endl;


  va = cdt.insert(Point(0.1, 0.5));
  vb = cdt.insert(Point(0.9, 0.5));
  vc = cdt.insert(Point(0.5, 0.1));
  vd = cdt.insert(Point(0.5, 0.9));
  ...
  mesher.refine_mesh();
  std::cout << "Number of vertices: " << cdt.number_of_vertices() << std::endl;
  auto it = cdt.finite_edges_begin();
  auto  end = cdt.finite_edges_end();
  for(; it != end; it++)
  {
      std::cout << cdt.segment(it->first, it->second) << std::endl;
  }
}
