...
class LinearForm(Form):
    ...
    def _single_assembly(self, retain_ints: bool) -> TensorLike:
        ...
        M = bm.sparse_coo_tensor(
            bm.empty((1, 0), dtype=space.itype, device=device),
            bm.empty((0,), dtype=space.ftype, device=device),
    ...
    def assembly(self, coalesce=True, retain_ints: bool=False,
                 return_dense=True) -> TensorLike:
        ...
        Returns:
            TensorLike[gdof,]. Batch is placed in the FIRST dimension for dense tensor,\
