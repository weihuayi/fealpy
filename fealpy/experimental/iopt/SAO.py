...
class SAO:
    ...
    def initialization(self):
        ...
        if bm.size(self.ub) == 1:
            ...
            for i in range(self.dim):
                high = self.ub[i]
                low = self.lb[i]
                X[:,i] = bm.random.rand(self.N)*(high - low) + low
    ...
    def cal(self):
        ...
        #均转为向量
        if bm.max(bm.size(self.ub)) == 1:
            self.ub = self.ub * bm.ones(self.dim)
            self.lb = self.lb * bm.ones(self.dim)
        ...
        for t in range(self.Max_iter):
            ...
            T = bm.exp(-t / self.Max_iter)
            # eq.(9)
            DDF = 0.35 + 0.25 * (bm.exp(t / self.Max_iter) - 1) / (bm.exp(1) - 1)
            # eq.(10)
            #融雪速率
            M = DDF * T
