...
class SquirrelSearchAlg(Optimizer):
    ...
    def run(self):
        ...
        for it in range(0, MaxIT):
            ...
            FSa = ((r1 > Pdp) * (FSa + dg * Gc * (gbest - FSa)) + 
            ...
            FSn[index3] = ((r3 > Pdp) * (FSn[index3] + dg * Gc * (gbest - FSn[index3])) + 
                           (r3 <= Pdp) * (lb + (ub - lb) * bm.random.rand(n3, dim)))
            # FSn = bm.concatenate((FSn2, FSn3), axis=0)
            FSn = FSn + (lb - FSn) * (FSn < lb) + (ub - FSn) * (FSn > ub)
            Sc = bm.sum((FSa - gbest) ** 2)
            Smin = 10 * bm.exp(bm.array(-6)) / 365 ** (it / (MaxIT / 2.5))
            if Sc < Smin:
                FSn = FSn + 0.01 * levy(N - 4, dim, 1.5) * (ub - lb)
                FSn = FSn + (lb - FSn) * (FSn < lb) + (ub - FSn) * (FSn > ub)

            x = bm.concatenate((gbest[None, :], FSa, FSn), axis=0)
            Div[0, it] = bm.sum(bm.sum(bm.abs(bm.mean(x, axis=0) - x)) / N)
            # exploration percentage and exploitation percentage
            D_pl[0, it] = 100 * Div[0, it] / bm.max(Div)
            D_pt[0, it] = 100 * bm.abs(Div[0, it] - bm.max(Div)) / bm.max(Div)
...
class DifferentialSquirrelSearchAlg(Optimizer):
    ...
    def run(self):
        ...
        for it in range(0, MaxIT):
            ...
            r1 = bm.random.rand(3, 1)
            FSa_t = ((r1 > Pdp) * (FSa + dg * Gc * (gbest - FSa - bm.mean(x, axis=0))) + 
                    (r1 <= Pdp) * (lb + (ub - lb) * bm.random.rand(3, dim)))
            ...
            r3 = bm.random.rand(n3, 1)
            FSn_Yt = ((r3 > Pdp) * (FSn[index3] + dg * Gc * (gbest - FSn[index3])) + 
                     (r3 <= Pdp) * (lb + (ub - lb) * bm.random.rand(n3, dim)))
            ...
            gbest_t = gbest + dg * Gc * (gbest - bm.mean(FSa, axis=0))
            gbest_t = gbest_t + (lb - gbest_t) * (gbest_t < lb) + (ub - gbest_t) * (gbest_t > ub)
            mask = self.fun(gbest_t[None, :]) < gbest_f
            gbest, gbest_f = bm.where(mask, gbest_t, gbest), bm.where(mask, self.fun(gbest_t[None, :]), gbest_f)
            
            FS_t = bm.concatenate((gbest[None, :], FSa_t, FSn_t), axis=0)
            Z_fit = self.fun(FS_t)
            mask = Z_fit < fit
