...
class StackStd(Module):
    ...
    """
    def __init__(self, center: Tensor, radius: float):
        super().__init__()
        self.center = center
        self.radius = radius

    def forward(self, p: Tensor):
        return (p[:, None, :] - self.center[None, ...]) / self.radius


class MultiLinear(Module):
    def __init__(self, in_features: int, out_features: int, parallel: int,
    ...
    def forward(self, x: Tensor):
        ret = torch.einsum('pio, npi -> npo', self.weight, x)
        return self.bias + ret
