...
from fealpy.typing import TensorLike, CoefLike
from fealpy.utils import is_scalar, is_tensor, fill_axis
...
def bilinear_integral(basis1: TensorLike, basis2: TensorLike, weights: TensorLike,
    ...
    if is_scalar(coef):
        return bm.einsum(f'q, c, cqid, cqjd -> cij', weights, measure, basis1, basis2) * coef
            ...
            # 检查 coef 的维度
            if coef.ndim >= 4 and coef.shape[-2] == coef.shape[-1]:  # 检查 coef 最后两个维度是否为 (n, n)
                # 进行矩阵形式的求和操作 (后两维为 n x n 的情况)
                return bm.einsum(f'q, c, cqid, cqjd, ...cqkl -> ...cijkl', weights, measure, basis1, basis2, coef)
            else:
                # 保留原有 fill_axis 逻辑，适用于其他维度情况
