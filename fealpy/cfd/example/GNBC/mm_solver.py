...
from fealpy.fem import TangentFaceMassIntegrator
...
class Solver():
    ...
    def CH_update(self, u_0, u_1, phi_0, phi_1 ,mv = None):
        dt = self.dt
        s = self.pde.s
        epsilon =self.pde.epsilon
        ...
        def phi_C_coef(bcs, index):
            return 2*dt*(2*u_1(bcs, index)-mv(bcs, index))
        ...
        def mu_BF_coef(bcs, index):
            ...
            result10 = bm.einsum('eld, ed->el', 2*u_1(bcs, index)-mv(bcs, index), tangent[index,:])
    ...
    def NS_update(self, u_0, u_1, mu_2, phi_2, phi_1, mv=None):
        dt = self.dt
        R = self.pde.R
        lam = self.pde.lam
        epsilon = self.pde.epsilon
        ...
        def u_C_coef(bcs, index):
            return 2*dt*(2*R*u_1(bcs, index) - mv(bcs, index))
