...
#include <torch/extension.h>  // 替换为标准扩展头文件
#include <vector>
#include <iostream>
...
at::Tensor ml_nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  const int col_blocks = CeilDiv(boxes_num, threadsPerBlock); // 使用自定义的CeilDiv
  ...
  // 使用at::empty创建张量并获取数据指针
  at::Tensor mask_tensor = at::empty({boxes_num * col_blocks}, boxes.options().dtype(at::kLong));
  unsigned long long* mask_dev = (unsigned long long*)mask_tensor.data_ptr<int64_t>();
  ...
  // 使用PyTorch的copy_方法
  cudaMemcpy(&mask_host[0], mask_dev, 
             sizeof(unsigned long long) * boxes_num * col_blocks,
             cudaMemcpyDeviceToHost);
