...
// // 原子操作实现（修正语法错误）
// template <typename T>
// __device__ static T gpuAtomicAdd(T* address, T val);  // 原声明保留 static

// template <>
// __device__ float gpuAtomicAdd<float>(float* address, float val) {  // 移除 static
//   return atomicAdd(address, val);
// }

// template <>
// __device__ double gpuAtomicAdd<double>(double* address, double val) {  // 移除 static
...
at::Tensor ROIAlign_forward_cuda(const at::Tensor& input,
  ...
  dim3 grid(std::min(CeilDiv((long)output_size, 512L), 4096L));  // 用自定义的CeilDiv
  dim3 block(512);

  if (output.numel() == 0) {
    // THCudaCheck(cudaGetLastError());
    C10_CUDA_CHECK(cudaGetLastError());  // 新的CUDA错误检查宏
    return output;
  }
...
at::Tensor ROIAlign_backward_cuda(const at::Tensor& grad,
  ...
  dim3 grid(std::min(CeilDiv((long)grad.numel(), 512L), 4096L));  // 用自定义的CeilDiv
  dim3 block(512);
