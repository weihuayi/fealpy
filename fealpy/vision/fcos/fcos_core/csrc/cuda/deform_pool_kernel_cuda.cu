...
#include <ATen/cuda/CUDAContext.h>  // 替代 THC/THC.h
#include <ATen/Dispatch.h>  // 类型分发宏依赖
  ...
  CUDA_KERNEL_LOOP(index, count)
  {
    ...
    for (int ih = 0; ih < sample_per_part; ih++)
    {
      for (int iw = 0; iw < sample_per_part; iw++)
      {
        ...
        // 替换4处特征梯度的atomicAdd
        scalar_t* ptr1 = offset_bottom_data_diff + bottom_index_base + y0 * width + x0;
        *ptr1 += q00 * diff_val;
        scalar_t* ptr2 = offset_bottom_data_diff + bottom_index_base + y1 * width + x0;
        *ptr2 += q01 * diff_val;
        scalar_t* ptr3 = offset_bottom_data_diff + bottom_index_base + y0 * width + x1;
        *ptr3 += q10 * diff_val;
        scalar_t* ptr4 = offset_bottom_data_diff + bottom_index_base + y1 * width + x1;
        *ptr4 += q11 * diff_val;
        ...
        // 替换2处偏移梯度的atomicAdd
        scalar_t* trans_ptr1 = bottom_trans_diff + (((n * num_classes + class_id) * 2) * part_size + part_h) * part_size + part_w;
        *trans_ptr1 += diff_x;
        scalar_t* trans_ptr2 = bottom_trans_diff + (((n * num_classes + class_id) * 2 + 1) * part_size + part_h) * part_size + part_w;
        *trans_ptr2 += diff_y;
      }
    }
  }
