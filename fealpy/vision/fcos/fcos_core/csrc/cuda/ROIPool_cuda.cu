...
// 原子操作实现
// template <typename T>
// __device__ T gpuAtomicAdd(T* address, T val);  // 移除 static

// template <>
// __device__ float gpuAtomicAdd<float>(float* address, float val) {  // 移除 __attribute__ 和 static
//   return atomicAdd(address, val);
// }

// template <>
// __device__ double gpuAtomicAdd<double>(double* address, double val) {  // 移除 __attribute__ 和 static
...
std::tuple<at::Tensor, at::Tensor> ROIPool_forward_cuda(const at::Tensor& input,
  ...
  dim3 grid(std::min(CeilDiv((long)output_size, 512L), 4096L));
  dim3 block(512);
  ...
  C10_CUDA_CHECK(cudaGetLastError());  // 添加分号
  return std::make_tuple(output, argmax);
}
...
at::Tensor ROIPool_backward_cuda(const at::Tensor& grad,
  ...
  dim3 grid(std::min(CeilDiv((long)grad.numel(), 512L), 4096L));
  dim3 block(512);
