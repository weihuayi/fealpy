...
from urllib.parse import urlparse  # 从标准库导入
...
HASH_REGEX = re.compile(r'-([a-f0-9]{8,})')  # 与旧版PyTorch的HASH_REGEX功能一致
...
def cache_url(url, model_dir=None, progress=True):
    ...
    parts = urlparse(url)  # 使用标准库的 urlparse
    if parts.fragment != "":
    ...
    if not os.path.exists(cached_file) and is_main_process():
        ...
        if hash_prefix is not None:
            ...
            if len(hash_prefix) < 6:
                hash_prefix = None
        download_url_to_file(url, cached_file, hash_prefix=hash_prefix, progress=progress)
    synchronize()
    return cached_file