...
class Standardize(Module):
    ...
    Parameters
        centers(Tensor): Tensor with shape (M, GD).

        radius(Union[Tensor, Any]): Tensor with shape (M, GD) or (GD, ), or any other object
...
class Distance(Module):
    """Calculate the distances between inputs and source points.
    Return with shape like (#Samples, #Sources).

    Parameters
        sources(Tensor): Tensor with shape (M, GD).
    """
    ...
    def gradient(self, p: Tensor):
        """Return the gradient with shape (#Samples, #Sources, #Dims)."""
...
class LeastSquare(Module):
    """Least square model for regression.

    Parameters
        ndof(int): Number of degrees of freedom.

        gd(int): Number of output dimensions.
