...
class RandomFeatureSpace(FunctionSpace):
    ...
    Construct a random feature space.

    Parameters
        in_dim(int): The geometry dimension of inputs.

        nf(int): Number of random features (basis functions).

        activate(Activationt): The activation function after the linear
               transformation.

        bound(tuple[float, float]):  Bounds of uniform distribution to initialize
               k, b in the each random feature.

        dtype: torch.dtype. Data type of inputs.
    ...
    @property
    def frequency(self):
        """ The "frequency" of the basis. Shape: (nf, GD). """
        return self.linear.weight

    @property
    def phrase(self):
        """ The "phrase" of the basis. Shape: (nf, ). """
        return self.linear.bias

    def number_of_basis(self):
        return self.nf

    def _linear(self, p: Tensor, *, index=_S) -> Tensor:
