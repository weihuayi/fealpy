...
class _PoU_Fn(Module):
    ...
    def flag(self, x: Tensor):
        """Support set flag. Return True if inside the support set, element-wise."""
        raise NotImplementedError

    def dn(self, x: Tensor, order: int) -> Tensor:
        """order-specified derivative."""
        if order == 0:
...
class PoU(TensorMapping):
    ...
    def flag(self, x: Tensor):
        """
        Return a bool tensor with shape (N, ) showing if samples are in
        the supporting area.
        """
        flag = (x >= -1-1e-16) * (x < 1-1e-16)
    ...
    def gradient(self, x: Tensor):
        """
        Return 
            gradient vector of the function, with shape (N, GD).
        """
    ...
    def hessian(self, x: Tensor):
        """
        Return  
            hessian matrix of the function, with shape (N, GD, GD).
        """
        N, GD = x.shape[0], x.shape[-1]
...
class PoUSpace(FunctionSpace, Generic[_FS]):
    ...
    Construct a function space by sub-spaces, using Partitions of Unity.

    Parameters
        space_factory(FunctionSpace): A function returning local spaces by their indices.

        centers(Tensor): position Tensor with shape (#parts, GD).

        radius(optional: Tensor, float, or sequence of floats): Radius of each partition. If a float is given, 
            all partitions have the same radius. If a sequence of floats is given, each partition has its own radius.
        
        pou(optional: PoU or list of PoU): Continuous real valued functions.
    ...
    def __init__(self, space_factory: SpaceFactory[_FS], centers: Tensor,
        ...
        for i in range(centers.shape[0]):
            part = PoULocalSpace(pou_fn=self.pou[i], space=space_factory, idx=i,
...
#Basis: {self.number_of_basis()}"""
    ...
    def partition_basis_slice(self, idx: int):
        """
        Parameters
            idx(int): The index of the partition.

        Return
        The index slice of the local basis function of the specified partition in the global model.
        """
        assert idx >= 0
    ...
    def from_uniform_mesh(cls, space_factory: SpaceFactory[_FS], uniform_mesh,
        ...
        """Construct PoU space from a uniform mesh.

        Parameters
            space_factory: A function generating spaces from indices.

            uniform_mesh: UniformMesh1d, 2d or 3d.

            part_loc: 'node' or 'cell'. Saying where the center of partition locates.

            pou: PoU, optional. Continuous real valued functions. Use the
...
class UniformPoUSpace(PoUSpace[_FS]):
    """
    Deprecated. Use PoUSpace.from_uniform_mesh instead.PoU space based on uniform mesh.
    ...
    Parameters
        ...
        pou(PoU): Continuous real valued functions. Use the indicator functions of partitions 
            by default (then continuous conditions between partitions are required).

        print_status(bool): Whether to print the status of the space.
    ...
    def collocate_sub_edge(self, n: int, edge_type=True):
        """Collocate interior points in every sub-boundaries of partitions.
            This is deprecated, use `sampler.InterfaceSampler` instead.

        Parameters
            n(int): The number of collocation points in each sub-boundary.

            edge_type(bool): If True, return the points in each sub-boundary,
                             else return the points in all sub-boundaries.

        Return
            Tensor with shape (N, #Subs, #Dims) if `edge_type`, else (N*#Subs, #Dims).

        Note
            Only works for 1-d boundary(edge).  
        """
    ...
    def sub_to_partition(self):
        """Return the relationship between sub-boundaries and partitions.

        Return
            Tensor with shape (#Subs, 2). Each row represents a sub-boundary,
        """
        location = self.location
        if location == 'node':
    ...
    def continue_matrix_0(self, points: Tensor, return_sparse=False):
        """Construct the matrix of 0th order continuity condition between
            partitions(WITHOUT PoU function).

        Parameters
            points(Tensor): Collocation points in sub-boundaries, must be with shape (N, #Subs, #Dims).

        Returns
            Tensor with shape (N*#Subs, #Basis).
        """
    ...
    def continue_matrix_1(self, points: Tensor, return_sparse=False):
        """Construct the matrix of 1st order continuity condition between\
            partitions(WITHOUT PoU function).
        ...
        Returns
            Tensor with shape (2*N*#Subs, #Basis).
        """
        assert points.ndim == 3 #(NVS, #Subs, #Dims)
        NVS = points.shape[0]
        NS = points.shape[1]
