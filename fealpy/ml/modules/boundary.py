...
class _BoxSetter():
    ...
    def set_box(self, box: List[float]):
        """Using a box boundary.

        Parameters
            box(list): a list containing floats to specify bounds in each axis.
                 For example, [0, 1, 0, 1].
        """
        assert len(box) >= 2
        assert len(box) % 2 == 0
...
class BoxDBCSolution1d(BoxDirichletBC):
    """A model wrapper of dirichlet boundary conditions in an 1d box area.
...
class BoxDBCSolution2d(BoxDirichletBC):
    """A model wrapper of dirichlet boundary conditions in an 2d box area.
           This is based on Theory of Functional Connection in 1d, 2d and 3d.

    Note
        !Not Fully Implemented! Now only 1d and 2d are supported.
    """
...
class BoxNBCSolution(Solution, _BCSetter, _BoxSetter):
    """A solution of problems with dirichlet boundary conditions in a box area.\
           This is based on Theory of Functional Connection in 1d, 2d and 3d.
