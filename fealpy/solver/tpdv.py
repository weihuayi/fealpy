...
from ..sparse import spdiags,csr_matrix
...
class DarcyForchheimerTPDv:
    ...
    def TPDv_IMEX(self):
        ...
        for ite in range(self.maxIt):
            ...
            if ite > 0 and resu[ite] / bm.linalg.norm(self.f) < self.tol:
                print("TPDv_IMEX Converged at iteration", ite)
                uoldAll, poldAll = unewAll, pnewAll
                break
    ...
    def TPDv_Newton(self):
        ...
        for ite in range(self.maxIt):
            ...
            if ite > 0 and (resu[ite] / bm.linalg.norm(self.f)) < self.tol:
                ...
                resu = resu[:ite + 1]
                resp = resp[:ite + 1]
                print("TPDv_Newton converged at iteration", ite)
                break
    ...
    def TPDv_PDHG(self):
        ...
        for ite in range(self.maxIt):
            ...
            if ite > 0 and resu[ite] / bm.linalg.norm(self.f) < self.tol:
                print("TPDv_PDHG Converged at iteration", ite)
                uoldAll, poldAll = unewAll, pnewAll
                break
