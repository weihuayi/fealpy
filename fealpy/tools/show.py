...
def showmultirate2(plot, k, N, errorMatrix, labellist, optionlist=None, lw=1,
        ms=4, propsize=10):
    """
    loglog的图，没有直线和收敛阶
    """
    ...
    for i in range(m):
        if len(N.shape) == 1:
            showrate2(axes, k, N, errorMatrix[i], optionlist[i], label=labellist[i], lw=lw, ms=ms)
        else:
            showrate2(axes, k, N[i], errorMatrix[i], optionlist[i], label=labellist[i], lw=lw, ms=ms)
    axes.legend(loc=3, framealpha=0.2, fancybox=True, prop={'size': propsize})
    return axes

def showrate2(axes, k, N, error, option, label=None, lw=1, ms=4):
    axes.set_xlim(left=N[0]/2, right=N[-1]*2)
    line0, = axes.loglog(N, error, option, lw=lw, ms=ms, label=label)
