...
class LaplaceSymetricForm:
    ...
    def get_matrix(self):
        ...
        if p == 1:
            ...
            D = np.ones((cell.shape[0], smldof), dtype=self.dtype)
            D[:, 1:] = (point[cell, :] - bc)/np.repeat(h, NV).reshape(-1, 1)
            G = np.array([(0, 0, 0), (0, 1, 0), (0, 0, 1)])
            for i in range(NC):
                K = B[:, cellLocation[i]:cellLocation[i+1]].T@G@B[:, cellLocation[i]:cellLocation[i+1]]
                M = np.eye(ldof[i]) - D[cellLocation[i]:cellLocation[i+1], :] @ B[:, cellLocation[i]:cellLocation[i+1]]
                K += M.T @ M
                dof = cell[cellLocation[i]:cellLocation[i+1]]
