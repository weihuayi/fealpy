...
class TravellingSalesmanProb:
    ...
    def __init__(self, pos):
        ...
        """
        self.pos = pos
        self.D = self._compute_distance_matrix(pos)
    ...
    def opt(self, N, MaxIT, opt_alg):
        ...
        """
        dim = self.D.shape[0]
        fobj = lambda x: self.path_len(x, self.D)
        x0 = initialize(N, dim, 1, 0)
        option = opt_alg_options(x0, fobj, (0, 1), N, MaxIters=MaxIT)
        self.optimizer = opt_alg(option)
        self.optimizer.run()
    
    def plot_route(self, pos):
        """
