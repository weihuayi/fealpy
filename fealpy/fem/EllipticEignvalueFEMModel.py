    ...
    def alg_3_5(self, maxit=None):
        ...
        while True:
            ...
            if (self.step > 0) and (final in idx):
                ...
                plt.close()
                self.savemesh(mesh,
                        self.resultdir + 'mesh_3_5_' + str(i+1) + '_' + str(NN) +'.mat')
        ...
        if (self.step > 0) and (0 in idx):
            ...
            if GD == 2:
                ...
                axes = Axes3D(fig)
            mesh.add_plot(axes, cellcolor='w')
            fig.savefig(self.resultdir + 'mesh_3_4_' + str(final) + '_' + str(NN) + 'H.pdf')
            plt.close()
            self.savemesh(mesh, self.resultdir + 'mesh_3_4_' + str(final) + '_' + str(NN) + 'H.mat')
