...
class OPCIntegrator(LinearInt, OpInt, CellInt):
    ...
    def fetch(self, space: _FS):
        ...
        bcs_dual = bm.array([[0.5,0.5,0.0],[0.5,0,0.5],[0.0,0.5,0.5]])
        ...
        return bcs, global_points, ws, phi, phi_dual, cm, index
    ...
    def assembly(self, space: _FS) -> TensorLike:
        ...
        coef = self.coef
        mesh = getattr(space, 'mesh', None)
        bcs, global_points, ws, phi,phi_dual, cm, index = self.fetch(space)
        if coef == 1:
            val_phi = phi
            val  =process_coef_func(coef, bcs=bcs, mesh=mesh, etype='face', index=index)
            reslut = bm.einsum('q, c, icqlk, cqik -> cil', ws, 1/3*cm, val_phi, phi_dual)*val
        else:
            val = process_coef_func(coef, bcs=bcs, mesh=mesh, etype='face', index=index)
            val_phi = bm.einsum('icqlk, cqdk -> icqld',  phi, val )
            reslut = bm.einsum('q, c, icqlk, cqik -> cil', ws, 1/3*cm, val_phi, phi_dual)
        return reslut
    ...
    def assembly(self, space: _FS) -> TensorLike:
        ...
        phi1_val = bm.einsum('cqlk, cqdk -> cqld',  phi1, val )
        phi2_val = bm.einsum('cqlk, cqdk -> cqld',  phi2, val )
        phi3_val = bm.einsum('cqlk, cqdk -> cqld',  phi3, val )
