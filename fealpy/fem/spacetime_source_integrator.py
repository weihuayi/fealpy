...
class SpaceTimeSourceIntegrator(LinearInt, SrcInt, CellInt):
    ...
    def phi_correction(self,p,cm,phi,gphi,coef):
        ...
        for _ in range(coef.ndim - 1):
            h = h[..., None]
        term_correction = bm.einsum('cqi...j, cq...j->cqi...' ,gphi, coef)
        phi_correction = phi + self.theta*h**p*s*term_correction
    ...
    def assembly(self, space: _FS, indices=None) -> TensorLike:
        ...
        return linear_integral(cphi, ws, cm, val, batched=self.batched)