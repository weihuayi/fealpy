...
'''!    	
	@Author: Ao Li
	@File Name: grad_pressure_integrator.py
	@bref 
	@ref 
'''  
...
(pI, \nabla v)
'''
class GradPressureIntegrator(LinearInt, OpInt, CellInt):
    ...
    def fetch(self, space: _FS):
        ...
        phi = space1.basis(bcs, index=index)
        gphi = space0.grad_basis(bcs ,index=index)
    ...
    def assembly(self, space: _FS) -> TensorLike:
        ...
        result = bilinear_integral(phi, gphi, ws, cm, val, batched=self.batched)
        #result = bm.einsum('cqik, cqjk, c, q -> cij', phi, gphi, cm, ws)
        return result
