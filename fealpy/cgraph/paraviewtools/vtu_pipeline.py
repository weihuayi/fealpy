...
class VTUScreenshot(CNodeType):
    r"""Render a styled VTU dataset and export to a PNG image.
    ...
    Outputs:
        png_path (string): Absolute path of the generated PNG file.
    """

    TITLE: str = "VTU截图"
    PATH: str = "后处理.ParaView"
    DESC: str = "渲染样式化的 VTU 数据并导出 PNG 图像。"
    INPUT_SLOTS = [
    ...
    OUTPUT_SLOTS = [
        PortConf("png_path", DataType.STRING, desc="导出的 PNG 文件路径", title="PNG 文件"),
    ]
    ...
    def run(
        ...
        # Write the VTK dataset to a temporary VTU file
        temp_vtu = target_path.parent / ".temp_pipeline.vtu"
