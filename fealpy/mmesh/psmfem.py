...
class PSMFEM(MM_monitor,MM_Interpolater):
    ...
    def _func_solver(self,U_bd = None):
        ...
        U0_inner = cg(H00,b0_inner - H01@U_bd[:,0],atol=1e-8,returninfo=True)[0]
        U1_inner = cg(H00,b1_inner - H01@U_bd[:,1],atol=1e-8,returninfo=True)[0]
        ...
        U_bd = cg(A0,b,atol=1e-8,returninfo=True)[0]
    ...
    def mesh_redistributor(self):
        ...
        for i in range(self.maxit):
            self.arc_length()
            v = self._func_solver(U_bd= U_bd)
            U_bd = v[self.isBdNode]
