...
class BilinearForm(Form[LinearInt]):
    ...
    def _scalar_assembly(self):
        ...
        for group_tensor, e2dofs_tuple in self.assembly_local_iterative():
            ue2dof = bm.split(e2dofs_tuple[0], e2dofs_tuple[1][1:-1])
            ve2dof = ue2dof
            ...
            f1 = lambda x: bm.repeat(x, x.shape[0])
            f2 = lambda x: bm.tile(x, (x.shape[0],))
            f3 = lambda x: x.flatten()
            I = bm.concatenate(list(map(f1, ue2dof)))
            J = bm.concatenate(list(map(f2, ve2dof)))
            group_tensor = bm.concatenate(list(map(f3, group_tensor)))
