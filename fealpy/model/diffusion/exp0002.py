...
class Exp0002(BoxMesher2d):
    """
    PDE model class for vector-valued variable coefficient diffusion equation with cubic polynomial coefficient.
    ...
    where:
        ...
        ρ(x,y) is a scalar cubic polynomial diffusion coefficient
    
    Exact solution:
        u1(x,y) = sin(πx)sin(πy)
        u2(x,y) = sin(2πx)sin(2πy)
    
    Cubic polynomial diffusion coefficient:
        ρ(x,y) = 2 + x + y + 0.5x³ + 0.5y³
    
    Source terms:
        f1 = -∇·(ρ∇u1) where ρ∇u1 needs to be computed with the cubic coefficient
        f2 = -∇·(ρ∇u2) where ρ∇u2 needs to be computed with the cubic coefficient
    """
