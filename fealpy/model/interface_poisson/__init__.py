...
class InterfacePoissonDataProtocol(Protocol):
    """Protocol interface for interface poisson PDE data components.
    
    Defines the recommended protocol interface for elliptic interface problem.
    ...
    This protocol suggests four main categories of methods that implementing classes may provide:
        ...
        2. Equation terms methods (exact solution, grdient, source terms and level function)
        3. Boundary condition methods (Dirichlet, Neumann, Robin types and interface jump condition)
    ...
    def level_function(p) -> TensorLike: ...
...
InterfacePoissonDataT = TypeVar('InterfacePoissonDataT', bound=InterfacePoissonDataProtocol)
...
DATA_TABLE = {
    ...
    "electronic": ("electronic_data_2d", "ElectronicData2D")
}