...
class LagrangeFESpace():
    """
    @brief 线性网格上 Lagrange 有限元空间
    """
    def __init__(self, mesh, p=1, spacetype='C', doforder=None):
        """
    ...
    def __str__(self):
        return "Lagrange finite element space on simplex mesh!"
...
class IntervalMeshCLFEDof(SimplexMeshCLFEDof):
    ...
    def entity_to_dof(self, 
        ...
        if etype in {'cell', 'edge', 1}:
            ...
            raise ValueError(f"Unsupported etype: {etype}. Supported types are: 
                    'cell', 'edge', 1, 'node', 'face', and 0.")
...
class TetrahedronMeshCLFEDof(SimplexMeshCLFEDof):
    ...
    def entity_to_dof(self, 
        ...
        if etype in {'cell', 3}:
            ...
            raise ValueError(f"Unsupported etype: {etype}. Supported types are:
                    'cell', 3, 'face', 2, 'edge', 1, 'node', and 0.")
