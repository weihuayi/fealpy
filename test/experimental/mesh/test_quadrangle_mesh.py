...
def test_shape_function_and_grad_shape_function():
    ...
    assert all(shape_function.reshape(-1)-bm.tensor([[0.62200847, 0.16666667, 0.16666667, 0.0446582 ],
                                                   ...
                                                   [0.0446582, 0.16666667, 0.16666667, 0.62200847]], dtype=bm.float64).reshape(-1)<1e-7)
    grad_shape_function = mesh.grad_shape_function(bcs)
    assert all(grad_shape_function.reshape(-1) - bm.tensor([[[[-0.78867513,-0.78867513],[-0.21132487,0.78867513],[0.78867513,-0.21132487],[0.21132487,0.21132487]]],
                                                           ...
                                                           [[[-0.21132487,-0.21132487],[-0.78867513,0.21132487],[0.21132487,-0.78867513],[0.78867513,0.78867513]]]],
...
def test_edge_geo():
    ...
    assert all(edge_frame[0].reshape(-1)-bm.tensor([[0., -1.],[-1., 0.],[ 1., 0.],[0., 1.]], dtype=bm.float64).reshape(-1)<1e-7)
    assert all(edge_frame[1].reshape(-1)-bm.tensor([[ 1., 0.],[ 0., -1.],[ 0., 1.],[-1., 0.]], dtype=bm.float64).reshape(-1)<1e-7)
    assert all(edge_unit_normal.reshape(-1)-bm.tensor([[0., -1.],[-1., 0.],[1., 0.],[0.,1.]], dtype=bm.float64).reshape(-1)<1e-7)
