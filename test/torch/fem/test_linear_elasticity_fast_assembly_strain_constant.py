...
def test_assembly(device):
    """
    Test the assembly of the strain constant matrix for linear elasticity.
    ...
    integrator_torch = LinearElasticityIntegrator(E=1.0, nu=0.3, device=device, method='fast_strain')

    # Assemble the strain constant matrix
    KK_torch = integrator_torch.fast_assembly_strain_constant(space=tensor_space)
