...
from  non_conforming_scalar_ve_space_2d_data import *
...
class TestConformingScalarVESpace2d():
    ...
    def test_dof(self, backend, data):
        ...
        space = NonConformingScalarVESpace2d(mesh, p=3)
        ...
        np.testing.assert_equal(is_boundary_dof, data["is_boundary_dof"])
        ...
        np.testing.assert_allclose(ipoint, data["ipoint"], atol=1e-10)
    ...
    def test_PI1(self, backend, data):
        bm.set_backend(backend)
        mesh = PolygonMesh.from_box([0,1,0,1],1,1)
        tmesh = TriangleMesh.from_box([0,1,0,1],1,1)
        mesh = PolygonMesh.from_mesh(tmesh)
        NC = mesh.number_of_cells()
        space = NonConformingScalarVESpace2d(mesh, p=1)
        PI1 = space.H1_project_matrix()
#        for i in range(NC):
...
if __name__ == "__main__":
    ...
    #t.test_PI1('numpy', 1)
    #t.test_PI1('pytorch', 1)
    #t.test_DOF('numpy', DOF[0])
    #t.test_DOF('numpy', 1)
    #t.test_DOF('pytorch', DOF[0])
    t.test_DOF('pytorch', 1)
