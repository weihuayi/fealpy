...
class ThermalMaterialProperties:
    ...
    def thermal_conductivity(self) -> TensorLike:
        return self.interpolation_model.calculate_property(self.rho, self.k0, self.kmin, self.penal)

    def thermal_conductivity_derivative(self) -> TensorLike:
        return self.interpolation_model.calculate_property_derivative(self.rho, self.k0, self.kmin, self.penal)

    def __repr__(self) -> str:
        rho_info = f"rho_shape={self.rho.shape}, rho_mean={bm.mean(self.rho):.4f}" if self.rho is not None else "rho=None"
        interpolation_model_name = self.interpolation_model.name

        return (f"ThermalMaterialProperties(thermal_conductivity={self.k0}, "
                f"kmin={self.kmin}, penal={self.penal}, "
                f"{rho_info}, "
