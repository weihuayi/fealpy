"""
UniformMesh3d 下比较不同线弹性积分子矩阵组装方法的效率
"""
...
origin = [0.0, 0.0, 0.0]
mesh = UniformMesh3d(
            extent=[0, n, 0, n, 0, n], h=h, origin=origin,
            ipoints_ordering='zyx', flip_direction=None,
            device='cpu')
GD = mesh.geo_dimension()
space= LagrangeFESpace(mesh, p=p)
tensor_space = TensorFunctionSpace(space, shape=(GD, -1)) # dof_priority
...
integrator_f3u = LinearElasticIntegrator(material=linear_elastic_material, 
                                        q=q, method='fast_3d_uniform')
integrator_f3u.keep_data(True)
t.send('准备')
...
# t.send('快速组装2')
KE_f3u = integrator_f3u.fast_assembly_uniform(tensor_space)
t.send('快速组装一致网格1')
KE_f3u_cache = integrator_f3u.fast_assembly_uniform(tensor_space)
t.send('快速组装一致网格2')
t.send(None)
error1 = bm.sum(bm.abs(KE[0] - KE_vu[0]))
# error2 = bm.sum(bm.abs(KE[0] - KE_fs[0]))
error3 = bm.sum(bm.abs(KE[0] - KE_f3u[0]))
