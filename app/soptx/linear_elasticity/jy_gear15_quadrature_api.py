...
# Jacobian 矩阵和第一基本形式的行列式
q = 2
qf = mesh.quadrature_formula(q)
bcs_quadrature, ws = qf.get_quadrature_points_and_weights()
J = mesh.jacobi_matrix(bcs_quadrature)           # (NC, NQ, GD, GD)
detJ = bm.linalg.det(J)                          # (NC, NQ)
mask_detJ = bm.abs(detJ) < 1e-2                  # (NC, NQ)
count_zero_detJ = bm.sum(mask_detJ)
print(f"count_zero_detJ: {count_zero_detJ}")
if count_zero_detJ > 0:
    zero_indices_detJ = bm.where(mask_detJ)
    print(f"共有 {count_zero_detJ} 个 detJ 接近零的积分点：")
    for c_idx, q_idx in zip(*zero_indices_detJ):
        value_detJ = detJ[c_idx, q_idx]
        print(f"  单元 {c_idx}, 积分点 {q_idx}: detJ = {value_detJ}")

G = mesh.first_fundamental_form(J)               # (NC, NQ, GD, GD)
detG = bm.linalg.det(G)                          # (NC, NQ)
mask_detG = bm.abs(detG) < 0.0096                # (NC, NQ)
count_zero_detG = bm.sum(mask_detG)
print(f"count_zero_detG: {count_zero_detG}")
if count_zero_detG > 0:
    zero_indices_detG = bm.where(mask_detG)
    print(f"共有 {count_zero_detG} 个 detG 接近零的积分点：")
    for c_idx, q_idx in zip(*zero_indices_detG):
        value_detG = detG[c_idx, q_idx]
        print(f"  单元 {c_idx}, 积分点 {q_idx}: detG = {value_detG}")
...
# 基于 Ansys 中的位移的应变张量
tgrad_ansys = bm.einsum('cqimn, ci -> cqmn', tgphi, uh_ansys_cell)           # (NC, NQ, GD, GD)
strain_ansys_u = (tgrad_ansys + bm.transpose(tgrad_ansys, (0, 1, 3, 2))) / 2 # (NC, NQ, GD, GD)
...
# 节点处的位移梯度
tphi = tensor_space.basis(bcs_quadrature)             # (1, NQ, tldof, GD)
N_gp = tphi[0, :, :, 0]  # (NQ, tldof)
N_gp_inv = bm.linalg.inv(N_gp)
strain_node = bm.einsum('nq, cqij -> cnij', N_gp_inv, strain)
...
bcs8 = (bm.tensor([[0, 1]], dtype=bm.float64), 
       bm.tensor([[0, 1]], dtype=bm.float64), 
       bm.tensor([[0, 1]], dtype=bm.float64))
tphi1 = tensor_space.basis(bcs1)                   # (1, 1, tldof, GD)
tphi2 = tensor_space.basis(bcs2)
tphi3 = tensor_space.basis(bcs3)
tphi4 = tensor_space.basis(bcs4)
tphi5 = tensor_space.basis(bcs5)
tphi6 = tensor_space.basis(bcs6)
tphi7 = tensor_space.basis(bcs7)
tphi8 = tensor_space.basis(bcs8)
tphi = bm.concatenate([tphi1, tphi2, tphi3, tphi4, 
                       tphi5, tphi6, tphi7, tphi8], axis=1) # (1, NCN, tldof, GD)
