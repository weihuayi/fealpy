...
from app.TSP.TSP_citys import gbest2route, soler_tsp_with_algorithm, printroute, soler_tsp_with_ant
import time
...
for algorithm, optimizer in optimizers.items():   
    gbest, gbest_f = soler_tsp_with_algorithm(optimizer, fobj, lb, ub, NP, citys.shape[0], test.D)  
    route, route_citys = gbest2route(gbest, citys)  
...
route_citys = bm.array([result['route_citys'].tolist() for result in results.values()])   
