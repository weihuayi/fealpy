...
for i in range(maxit):
    ...
    # print("element:", np.max(np.abs(bm.to_numpy(KK_strain[0]) - KK_strain_old[0])))
    ...
    print("FF:", FF.shape, "\n", FF[0])
    # print("FF_old:", FF_old[0].shape, "\n", FF_old[0])
    # print("element:", np.max(np.abs(bm.to_numpy(FF[0]) - FF_old[0].reshape(-1))))
    ...
    u_exact = space_old.interpolate(solution)
...
print("order_old:\n", np.log2(errorMatrix[0,:-1]/errorMatrix[0,1:]))
