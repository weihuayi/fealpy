...
def source(points: TensorLike) -> TensorLike:
    ...
    val[..., 0] = 20/9 * x * (1 - x) + 10/9 * (1 - 2 * y)
    val[..., 1] = 10/9 * y * (1 - y) + 20/9 * (1 - 2 * x)
...
for i in range(maxit):
    ...
    integrator_bi_independent = LinearElasticityIntegrator(E=1.0, nu=0.3, 
                                           method='fast_stress', q=5)
    
    # 与单元有关的组装方法
    KK_strain = integrator_bi_dependent.assembly(space=tensor_space)
    # 与单元无关的组装方法
    KK_strain = integrator_bi_independent.fast_assembly_stress_constant(space=tensor_space)
