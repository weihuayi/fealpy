...
gmsh.initialize()
gmsh.model.add('EPI_IGCT')
gmsh.merge('case_5_EPI_IGCT.msh')
...
gmsh.view.addModelData(bgview, 0, "EPI_IGCT", "NodeData", gmesh.vtags,
        nodesize[:,None])
mesh.nodedata["nodesize"] = nodesize
mesh.celldata["elementsize"] = elementsize

mesh.to_vtk(fname='test_EPI.vtu')
gmsh.model.add("EPI_IGCT")
...
gmsh.model.geo.addPoint(250,525,0,tag=5)
gmsh.model.geo.addPoint(0,525,0,tag=6)
...
mesh = TriangleMesh(node,cell)
mesh.to_vtk(fname='area_EPI_adaptive.vtu')
