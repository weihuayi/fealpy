...
for i in range(300):
    ...
    r_i_s, r_j_s = mesh.nodedata["position"][bm.array(i_s)], mesh.nodedata["position"][bm.array(j_s)]
    ...
    #标记
    wall_mask = bm.where(jnp.isin(mesh.nodedata["tag"], bm.array([1, 3])), 1.0, 0.0)
    fluid_mask = bm.where(mesh.nodedata["tag"] == 0, 1.0, 0.0)
