...
class example_1:
    ...
    def q_solution(self, space, time):
        ...
        x2 = self.x2
        t = self.t
        result = sp.lambdify([x1,x2,t], self.q, self.manager)
    ...
    def f_fun(self, space, time, index=None):
        ...
        x1 = self.x1
        x2 = self.x2
        self.f = sp.diff(y, t, 2) + sp.diff(p[0], x1) + sp.diff(p[1], x2) + self.c*y - u
    ...
    def y_d_fun(self,  space, time):
        ...
        self.y_d = y - sp.diff(z, t, 2) - sp.diff(q[0], x1) - sp.diff(q[1], x2) - self.c*z
