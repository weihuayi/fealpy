...
# CN 格式
def parabolic_fd2d_CrankNicholson(pde, mesh, time):
        NT = time.NL - 1
        dt = time.dt
        hx = mesh.hx
        hy = mesh.hy
        ...
        A_0 = diags([1 + dt * (cx + cy)], [0], shape=(NN, NN), format='coo')

        val = np.broadcast_to(-dt * cy / 2, (NN - n1,))
        I = k[1:, :].flat
        J = k[0:-1, :].flat
        A_0 += coo_matrix((val, (I, J)), shape=(NN, NN), dtype=np.float64)
        A_0 += coo_matrix((val, (J, I)), shape=(NN, NN), dtype=np.float64)

        val = np.broadcast_to(-dt * cx / 2, (NN - n0,))
        I = k[:, 1:].flat
        J = k[:, 0:-1].flat
        A_0 += coo_matrix((val, (I, J)), shape=(NN, NN), dtype=np.float64)
        A_0 += coo_matrix((val, (J, I)), shape=(NN, NN), dtype=np.float64)
        ...
        B = diags([1 - dt * (cx + cy)], [0], shape=(NN, NN), format='coo')
        ...
        for i in range(NT):
            ...
            F[:] = dt / 2 * (pde.source(p, ct) + pde.source(p, nt))
