...
class MeshOptimizationBase:
    def __init__(self, lr=0.001, max_epochs=10000, error_threshold=1e-3):
        self.lr = lr
        self.max_epochs = max_epochs
        self.error_threshold = error_threshold
        self.optimizers = {}
        self.parameters = {}
        self.prev_parameters = {}  # 用于存储上一次迭代的参数值
    ...
    def add_optimizable(self, name, param, optimizer_type=torch.optim.Adam):
        ...
        self.prev_parameters[name] = param.detach().clone()  # 初始化历史值
    ...
    def optimize(self):
        ...
        for step in range(self.max_epochs):
            ...
            for name, param in self.parameters.items():
                param_diff += bm.linalg.norm(param - self.prev_parameters[name])
...
class MeshOptimizationBase:
    ...
    def optimize(self):
        ...
        for step in range(self.max_epochs):
            ...
            if step == 18800:
                print("Debugging step reached, stopping optimization.")
            param_diff = 0.0
            for name, param in self.parameters.items():
...
# if __name__ == "__main__":
#     origin_mesh = TetrahedronMesh.from_box(nx=4, ny=4, nz=4)
#     # 定义权重，例如 node 权重为 1.0
#     weights = {"node": 1.0}
#     deformer = MeshBallDeformation(origin_mesh, s=1, alpha=0.5, weights=weights, weight_amips=10., weight_ball=1.0)
#     optimized_mesh = deformer.optimize()
#     optimized_mesh.to_vtk(fname="optimized_mesh_ball_energy.vtu")
#     pickle.dump(optimized_mesh, open("optimized_mesh_ball_energy.pkl", "wb"))
#     deformer.plot_mesh(optimized_mesh, title="Optimized Mesh (Ball Deformation)")