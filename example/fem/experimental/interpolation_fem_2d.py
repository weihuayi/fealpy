...
from fealpy.functionspace.cr_conforming_fe_space_2d import CrConformingFESpace2d
...
for i in range(maxit):
    mesh = TriangleMesh.from_box([0,1,0,1], nx, nx)
    ...
    space = CrConformingFESpace2d(mesh, p=p, m=m)
    ...
    errorMatrix[0, i] = mesh.error(flist[0], fI, q=p+1)
    print('error1')
    errorMatrix[1, i] = mesh.error(flist[1], ug1val, q=p+1)
    print('error2')
    errorMatrix[2, i] = mesh.error(flist[2], ug2val, q=p+1)
    print('error3')
    errorMatrix[3, i] = mesh.error(flist[3], ug3val, q=p+1)
    print('error4')
    print(errorMatrix)
