...
from fealpy.fem import LinearRecoveryAlg, recovery_alg
...
class Brittle_Facture_model():
    ...
    def is_force_boundary(self, p):
        ...
        @brief 标记施加力的节点
        """
        return (np.abs(p[..., 1] - 250) < 1e-12)&(np.abs(p[..., 0]-470)<1e-5)
...
mesh = TriangleMesh.from_box(box=[0, 500, 0, 500], nx=100, ny=100,
        threshold=no_mesh)
mesh.to_vtk(fname='test_Lmesh.vtu')
...
disp = model.force_boundary_disp()
...
for i in range(len(disp)-1):
    ...
    while k < 100:
        ...
        isTNode = model.is_force_boundary(node)
